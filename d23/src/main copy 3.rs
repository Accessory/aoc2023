...
fn run(input_file: &str) {
    ...
    while let Some(state) = queue.pop_back() {
        ...
        if let Some(connection) = connections.get(&state.position) {
            ...
            if !next_way.contains(&state.position) {
...
fn run2(input_file: &str) {
    ...
    while let Some(state) = queue.pop_back() {
        if state.position == goal {
            ...
            println!("{} - {}", state.steps, state.way.len());
            result.push(state);
            continue;
        }

        if let Some(connection) = connections.get(&state.position) {
            if !state.way.contains(&connection.0) {
                let mut next_way = state.way.clone();
                next_way.extend(connection.1.iter());
                let next_state = State {
                    position: connection.0,
        ...
        if next_posibilities.len() == 1 {
            if line.is_none() {
                line = Some((state.position, vec![state.position]));
            } else {
                line.as_mut().unwrap().1.push(state.position);
            }
        } else {
    ...
    for (_, (_, connection)) in connections {
        for con in connection {
            grid.set_from_point(&con, 'X');
        }
    }

    let max = result.iter().max_by_key(|f| f.steps).unwrap();

    // for pos in &max.way {
    //     grid.set_from_point(&pos, 'O');
    // }
    ...
    // Result is 6378
    let max_result = result.iter().map(|r| r.steps).max().unwrap();
    println!("Result of part 2 is {} ,{}", max_result, max.way.len());
}
