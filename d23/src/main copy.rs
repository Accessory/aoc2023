...
fn run(input_file: &str) {
    ...
    while let Some(state) = queue.pop_front() {
        ...
        let next_posibilities = match *grid.get_from_point(&state.position).unwrap() {
        ...
        for next in next_posibilities {
            let c = *grid.get_from_point(&next).unwrap();
            if c != '#' && !next_way.contains(&next) {
...
fn run2(input_file: &str) {
    ...
    const START: GridPoint = GridPoint { x: 1, y: 0 };
    // Parse
    let mut data = parse_into_char_vector_vector(input_file);
